...
class EXP2File:
    ...
    def decode(self, fileBytes):
        ...
        if magic != "ETC EXP II":
            self.logger.error(
                'decode(): Magic string has lost its magic. Expected "ETC EXP II", got "{}".'.format(
                    magic
                )
            )
            if self.strict == True:
        ...
        if int.from_bytes(recordType, "little") != 0:
            self.logger.error(
                "decode(): First record in index table is not of type 0000. (Got {})".format(
                    recordType.hex()
                )
            )
            if self.strict == True:
            ...
            self.logger.error(
                "decode(): First record in index table has non-null index value. (Got {})".format(
                    recordIndex.hex()
                )
            )
            if self.strict == True:
    ...
    def encode(self, data):
        ...
        )  # index offset is always 0x20 in the console, while it's technically relocatable the console may not like it elsewhere
    ...
    def parseRecord(self, record, fileBytes):
        ...
        if calculatedChecksum != checksum:
            self.logger.warning(
                "Record type {}, index {} checksum incorrect. Expected {} got {}.".format(
                    recordType.hex(), checksum.hex(), calculatedChecksum.hex()
                )
            )
    ...
    def parseIndexChunk(self, chunk, fileBytes):
        ...
        if (size % 16) != 0:
            self.logger.error(
                "parseIndexChunk(): Chunk is not a multiple of 16 bytes. Aborting..."
            )
            return False
        ...
        for i in range(numRecords):
            recordBytes = chunk[
                i * 16 : (i * 16) + 16
            ]  # chop out 16 byte sections at a time
